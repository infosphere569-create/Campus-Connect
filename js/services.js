...
export async function createPost(data){if(!db)throw new Error('Firebase is not configured.');
  // firestore.rules requires the field "authorId" on create (request.resource.data.authorId==request.auth.uid).
  // Callers historically passed "authorUid" instead, which silently failed every publish with permission-denied.
...
export async function addComment(postId,data){if(!db)throw new Error('Firebase is not configured.');
  // Same authorId/authorUid mismatch as createPost -- firestore.rules requires "authorId" on comment create.
  const authorId=data.authorId||data.authorUid;
