...
async function comments(postId){let comments=[],loadFailed=false;if(db){try{comments=await getPostComments(postId)}catch(e){loadFailed=true}}
 const body=loadFailed
  ...
  :(comments.length?comments.map(c=>`<div class="comment"><div class="avatar sm">${initials(c.authorName)}</div><div><strong>${escapeHtml(c.authorName)}</strong><div>${escapeHtml(c.text)}</div><span class="small muted">${timeAgo(c.createdAt)}</span></div></div>`).join(''):'<div class="empty"><i data-lucide="message-circle"></i><h3>No comments yet</h3><p>Start the conversation.</p></div>');
